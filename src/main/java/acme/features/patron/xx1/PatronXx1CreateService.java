...
@Service
public class PatronXx1CreateService implements AbstractCreateService<Patron, Xx1>{
	...
	public void validate(final Request<Xx1> request, final Xx1 entity, final Errors errors) {
        ...
        if(!errors.hasErrors("code")) {
            ...
            errors.state(request, result, "code", "patron.xx1.form.error.code-date");
        }
        
        if(!errors.hasErrors("code")) {
        	errors.state(request, this.repository.findXx1ByCode(entity.getCode()) == null, "code", "patron.xx1.form.error.not-unique");
        }
		...
		if(!errors.hasErrors("xx3")) {
        	...
        	errors.state(request, !spamDetector.isSpam(entity.getXx3(), softSpam, softThreshold, hardSpam, hardThreshold), "xx3", "patron.xx1.form.error.spam");
        }
		...
		if(!errors.hasErrors("xx4")) {
        	...
        	final List<String> hardSpam = new ArrayList<String>(Arrays.asList(hp));
        	final Double hardThreshold = configuration.getStrongSpamThreshold();
        	errors.state(request, !spamDetector.isSpam(entity.getXx4(), softSpam, softThreshold, hardSpam, hardThreshold), "xx4", "patron.xx1.form.error.spam");
        }
		...
		if(!errors.hasErrors("xx51")) {
        	...
        	errors.state(request, startPeriod.after(calendar.getTime()), "xx51", "patron.xx1.form.error.start-period-not-enough");
        }
		...
		if(!errors.hasErrors("xx52") && entity.getXx51()!=null) {
        	...
        	errors.state(request, endPeriod.after(moment), "xx52", "patron.xx1.form.error.end-period-one-week-before-start-period");
        }
		...
		if(!errors.hasErrors("xx6")) {
            ...
            errors.state(request, isCorrect, "xx6", "patron.xx1.form.error.incorrect-currency");
        }
        
        if(!errors.hasErrors("xx6")) {
            errors.state(request, entity.getXx6().getAmount() >= 0.0, "xx6", "patron.xx1.form.error.negative-budget");
        }
        
        if(!errors.hasErrors("items")) {
            final String itemCode = request.getModel().getString("items");
            final Item item = this.repository.findOneItemByCode(itemCode);
        	if (itemCode.equals("No Xx1s available")) {
        ...
        if(!errors.hasErrors("items")) {
            final String itemCode = request.getModel().getString("items");
            final Item item = this.repository.findOneItemByCode(itemCode);
            errors.state(request, !itemCode.equals(""), "items", "patron.xx1.form.error.item-null");
            if (!itemCode.equals("")) {
                errors.state(request, item != null, "items", "patron.xx1.form.error.item-does-not-exist");
            }
        }
	...
	public void unbind(final Request<Xx1> request, final Xx1 entity, final Model model) {
		...
		List<String> possible = this.repository.findAllPossibleItemCodes();
	...
	public Xx1 instantiate(final Request<Xx1> request) {
		...
		Principal principal = PrincipalHelper.get();
		Patron patron = this.repository.findOnePatronByUsername(principal.getUsername());
